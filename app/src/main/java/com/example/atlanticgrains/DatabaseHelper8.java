...
public class DatabaseHelper8 extends SQLiteOpenHelper {
    ...
    public Integer updateResponse(String id, String response) {
        int result = 0;
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        System.out.println("puta? " + id + "/" + response);
        contentValues.put("response", response);
        result = db.update(TABLE_NAME, contentValues, "id=?", new String[]{id});
        return result;
    }

    public Cursor getAllDataByModule(String fromModule) {
        Cursor cursor;
        SQLiteDatabase db = this.getWritableDatabase();
        cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE from_module= ?", new String[]{fromModule});
        return cursor;
    }
